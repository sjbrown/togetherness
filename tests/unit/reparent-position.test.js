 ...
 * addToyDom), and (2) the moved toy's position gets converted into the
 ...
 * branch: findDropTarget → reparentToyDom → getGeom the tray →
 * applyMoveDom the moved toy at (dropX - trayGeom.x, dropY - trayGeom.y).
 */
...
import {
  addToy, findDropTarget, reparentToyDom, getGeom, applyMoveDom,
...
// module doc above.
function dropIntoTray(ydoc, layerEl, draggedId, dropX, dropY) {
  const dropTrayId = findDropTarget(layerEl, draggedId, dropX, dropY)
  if (!dropTrayId) return null
  const movedEl = reparentToyDom(layerEl, draggedId, dropTrayId)
...
describe('drop-position rebase — real tray_sum + dice_d6 assets', () => {
  ...
  test('tray_sum places at its real 220x130 native size, not a forced square', async () => {
    ...
    await addToy(ydoc, layerEl, { id: 'tray1', toyType: 'tray_sum', x: 300, y: 300, color: '#fff' })
  ...
  test('dice_d6 places at its real 80x100 native size', async () => {
    const ydoc = new Y.Doc()
    const layerEl = freshLayer()
    await addToy(ydoc, layerEl, { id: 'die1', toyType: 'dice_d6', x: 100, y: 100, color: '#fff' })
  ...
  test('dropa die into a tray converts position to the trays local space and inside the trays visible viewBox', async () => {
    ...
    await addToy(ydoc, layerEl, { id: 'tray1', toyType: 'tray_sum', x: 400, y: 375, color: '#fff' })
    // die dropped loose on the table, nowhere near the tray at first
    await addToy(ydoc, layerEl, { id: 'die1', toyType: 'dice_d6', x: 50, y: 50, color: '#fff' })

    // drop it at table point (350, 340) — visually inside the tray's rendered box
    const { dropTrayId, movedEl } = dropIntoTray(ydoc, layerEl, 'die1', 350, 340)

    expect(dropTrayId).toBe('tray1')
    const movedSvg = movedEl.querySelector('svg')
  ...
  test('without the rebase, the die\u2019s stale table-space position would NOT be visible in the tray\u2019s local viewBox (regression guard)', async () => {
    ...
    await addToy(ydoc, layerEl, { id: 'die1', toyType: 'dice_d6', x: 900, y: 900, color: '#fff' })

    const dropTrayId = findDropTarget(layerEl, 'die1', 350, 340)
    const movedEl = reparentToyDom(layerEl, 'die1', dropTrayId) // reparent only, no rebase

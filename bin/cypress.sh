...
if [ -z "$CYPRESS_BASE_URL" ]; then
  ...
  echo ""
  echo "CYPRESS_BASE_URL=http://172.17.0.1:8000 bin/cypress.sh"
  echo "CYPRESS_BASE_URL=http://172.17.0.3 bin/cypress.sh"
  echo "CYPRESS_BASE_URL=http://host.docker.internal:8000 bin/cypress.sh"
  exit 1
fi

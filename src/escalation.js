/**
 * escalation.js — What to DO once conflict.js has detected a conflict.
 *
 * This is the resolution half: deciding who wins,
 * reverting the loser's own contribution out of the shared document, and
 * forking whatever that revert can't recover.
 *
 * Three pieces:
 *   - resolveConflictWinner — authority ordering for a detected pair,
 *     using each bundle's self-reported authorId against joinSequence.
 *
 *   - revertBundle — delete the loser's own structural insertions, AND
 *     restore any pre-existing content the loser's own commit removed.
 *
 ...
 *     snapshot) and so needs branch escalation on top.
 */
 ...
 * matching revert snapshot is available — the earlier
 * "clone-before-delete" capture in commitEnvelope.
 *
 * A mismatch (see hasMatchingSnapshot) means restoration genuinely isn't
 * available for this bundle.
 * Call needsEscalation before restoreFromSnapshot, since this latter
 * function CONSUMES the snapshot on success.
 *
 ...
 * closing it fully needs a solution
 * (coordination?, fork?, detect by DOM id?)
 */
 ...
 *
 * revertBundle's insertion-delete is idempotent by construction:
 *
 * Deletion:
 ...
 * bundle, redundantly, with no coordination.
 *
 * Insertion:
 * Idempotency here is more tricky than deletion. Restoring is an INSERT,
 * and Yjs never deduplicates inserts by content, so two peers each
 * independently restoring the same snapshot would create two separate,
 * genuinely distinct toys.
 * Restoration is idempotent against sequential reprocessing (consumes
 * the snapshot on success) but not against two peers both restoring
 * before either's eviction has synced — see restoreFromSnapshot's doc comment
 */
